...
public class ProcessManager {
    ...
    private ArrayList<DA_Byzantine_RMI> processes;
    ...
    public void startServer(){
        ...
        for (String url : urls){
            try{
                DA_Byzantine_RMI process;
                if (isProcessLocal(url)){
                    process = new DA_Byzantine(urls, processIndex);
                    LOGGER.debug("Process " + processIndex + " is local.");
                    new Thread((DA_Byzantine)process).start();
                    Naming.bind(url, process);
                } else {
                    process = (DA_Byzantine_RMI)Naming.lookup(url);
